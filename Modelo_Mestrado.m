function [T,S,I,R] = Modelo_Mestrado(beta,gamma,mu,rei,S0,I0,p, tV, MaxTime)
%
% 
%
...
[t, pop]=ode45(@Diff_8_1,[0 tV],[S I R],options,[beta gamma mu rei 0]);
T=t; S=pop(:,1); I=pop(:,2); R=pop(:,3);

[t, pop]=ode45(@Diff_8_1,[tV MaxTime],pop(end,:),options,[beta gamma mu rei p]);
...
function dPop=Diff_8_1(~,pop, parameter)

beta=parameter(1); gamma=parameter(2); mu=parameter(3);  p=parameter(4); rei=parameter(5);
S=pop(1); I=pop(2); R=pop(3);

dPop=zeros(3,1);

dPop(1)= mu*(1-p) -beta*S*I - mu*S + gamma*I;
